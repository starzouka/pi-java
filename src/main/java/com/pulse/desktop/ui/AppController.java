...
public class AppController {
    ...
    @FXML
    private void openTopAdmin() {
        openRoute("admin_categories");
    }
    ...
    private void openRoute(String routeId) {
        String requestedRouteId = routeId;
        String baseRouteId = routeId;
        String payload = null;

        int sep = routeId == null ? -1 : routeId.indexOf("::");
        if (sep > 0) {
            baseRouteId = routeId.substring(0, sep);
            payload = routeId.substring(sep + 2);
        }

        if ("front_game_detail".equals(baseRouteId) && payload != null && !payload.isBlank()) {
            try {
                RouteContext.setSelectedGameId(Integer.parseInt(payload.trim()));
            } catch (NumberFormatException ignored) {
            }
        }

        if (isAdminOnlyRoute(baseRouteId) && !"ADMIN".equals(SessionContext.currentRole())) {
            AlertUtils.warning("Acces", "Veuillez vous connecter en tant qu'admin.");
            baseRouteId = "front_login";
        }

        RouteDefinition route = routes.getOrDefault(baseRouteId, routes.get("front_home"));
        String fxmlPath;
        ...
        try {
            ...
            AlertUtils.error("Navigation", "Impossible d'ouvrir la page " + requestedRouteId + "\n" + ex.getMessage());
        }
    }

    private static boolean isAdminOnlyRoute(String routeId) {
        if (routeId == null) {
            return false;
        }
        return routeId.startsWith("admin_") || "front_users_placeholder".equals(routeId);
    }
    ...
    private void handleActionRoute(String routeId) {
        switch (routeId) {
            ...
            case "front_payment_checkout", "front_payment_checkout_session":
                ...
                break;
            case "front_profile_2fa_setup", "front_profile_2fa_enable", "front_profile_2fa_disable",
    ...
    private void refreshSessionDependentUi() {
        ...
        if (navAdminButton != null) {
            navAdminButton.setManaged(adminVisible);
            navAdminButton.setVisible(adminVisible);
        }
    }
    ...
    private void refreshTopNavActive() {
        ...
        if (navAdminButton != null) {
            navAdminButton.getStyleClass().remove("topbar__item--active");
        }
        ...
        } else if (isAdminRoute(activeRouteId) && navAdminButton != null) {
            navAdminButton.getStyleClass().add("topbar__item--active");
        }
    }
    ...
    private void buildSidebar() {
        ...
        addLinks(sectionCaptainLinks, List.of(
                "front_captain_team_manage", "front_captain_members", "front_captain_tournaments",
                "front_captain_products"));
        addLinks(sectionOrganizerLinks, List.of(
                "front_organizer_requests", "front_organizer_tournaments", "front_organizer_matches",
    ...
    private static boolean isHomeRoute(String route) {
        return List.of(
                ...
                "front_two_factor_challenge").contains(route);
    }
    ...
    private static boolean isTournamentsRoute(String route) {
        return route.startsWith("front_tournament")
                ...
                || List.of("front_organizer_registrations", "front_captain_tournaments",
                        "front_captain_team_tournaments", "front_captain_requests")
                        .contains(route);
    }

    private static boolean isGamesRoute(String route) {
        return route.startsWith("front_game") || "admin_games".equals(route);
    }
    ...
    private static boolean isShopRoute(String route) {
        return List
                .of("front_shop", "front_cart", "front_checkout", "front_orders", "front_order_detail",
    ...
    private static boolean isTeamsRoute(String route) {
        return route.startsWith("front_team")
                ...
                || List.of("front_players", "front_friends", "front_my_teams", "front_my_requests",
                        "front_captain_members", "front_captain_invite", "front_captain_team_create",
                        "front_captain_team_manage")
                        .contains(route);
    }

    private static boolean isAdminRoute(String route) {
        if (route == null) {
            return false;
        }
        return route.startsWith("admin_") || "front_users_placeholder".equals(route);
    }
}
