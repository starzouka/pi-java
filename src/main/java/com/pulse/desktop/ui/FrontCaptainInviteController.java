...
public class FrontCaptainInviteController implements RouteAwarePage {
    ...
    private void sendInvite(int invitedUserId, String message) {
        ...
        try {
            ...
            if (result.ok()) {
                notifyInvitationByEmail(invitedUserId, message);
                refreshAll();
            }
        } catch (SQLException ex) {
    ...
    private void notifyInvitationByEmail(int invitedUserId, String message) {
        if (!mailService.isConfigured() || activeTeam == null) {
            return;
        }
        try {
            TeamModuleRepository.TeamInvitationEmailData data = repository.loadTeamInvitationEmailData(activeTeam.teamId(), invitedUserId);
            if (data == null) {
                return;
            }
            String toEmail = trimOrNull(data.invitedEmail());
            if (toEmail == null) {
                return;
            }

            mailService.sendTeamInvitationEmail(
                    toEmail,
            ...
            String current = feedbackLabel.getText() == null ? "" : feedbackLabel.getText().trim();
            if (current.isBlank()) {
                feedbackLabel.setText("Invitation enregistree. Email non envoye.");
            } else {
                feedbackLabel.setText(current + " Email non envoye.");
            }
        }
    }

    private static String buildInviteActionUrl() {
        String base = AppConfig.webBaseUrl();
        if (base == null || base.isBlank()) {
            return null;
        }
        String normalized = base.endsWith("/") ? base.substring(0, base.length() - 1) : base;
        return normalized + "/my-teams";
    }
